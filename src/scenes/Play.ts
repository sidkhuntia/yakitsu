...
export default class Play extends Phaser.Scene {
  ...
  create() {
    ...
    this.input.keyboard!.on("keydown", (event: KeyboardEvent) => {
      ...
      if (ok) {
        ...
        // Play avatar hit animation
        if (
          this.avatar &&
          this.avatar.anims.currentAnim?.key !== "avatar-hit"
        ) {
          this.avatar.play("avatar-hit");
          // Listen for the completion of the 'avatar-hit' animation
          this.avatar.once(
            Phaser.Animations.Events.ANIMATION_COMPLETE,
            (anim: Phaser.Animations.Animation) => {
              // Ensure the completed animation is 'avatar-hit' and the game isn't over
              if (anim.key === "avatar-hit" && !this.gameOverTriggered) {
                this.avatar.play("avatar-run"); // Revert to run animation
              }
            }
          );
        }
  ...
  getNextWord(): string {
    ...
    if (level === 1 && this.mediumWords.length)
  ...
  createSprites() {
    ...
    if (!this.anims.exists("avatar-hit")) {
      // Assuming 'Take Hit.png' has 4 frames for the hit animation.
      // Adjust 'end: 3' if your spritesheet has a different number of frames.
      this.anims.create({
        key: "avatar-hit",
        frames: this.anims.generateFrameNumbers("avatar_hit", {
          start: 0,
          end: 3,
        }),
        frameRate: 10, // Adjust frameRate as needed
  ...
  createText() {
    ...
    this.typedText = this.add
      ...
      .setOrigin(0, 0.5)
      .setDepth(20); // Ensure text is above everything
  ...
  update() {
    if (!this.gameOverTriggered && !this.settingsModalOpen) {
      ...
      if (
        Phaser.Geom.Intersects.RectangleToRectangle(avatarBounds, monsterBounds)
      ) {
        this.loseLife();
  ...
  updateWordDisplay() {
    ...
    const totalWidth =
      this.typedText.width + this.caretText.width + this.remainingText.width;
    ...
    if (Math.random() < 0.05) {
      ...
      if (!this.powerUpSprite) {
        this.powerUpSprite = this.add
          .image(this.monster.x, this.monster.y - 40, this.powerUpType)
          .setOrigin(0.5)
          .setScale(1.5)
  ...
  createFullscreenButton() {
    ...
    this.fullscreenBtn = this.add
      .dom(this.scale.width - 40, 40, btn)
      .setOrigin(1, 0)
      .setDepth(100);
